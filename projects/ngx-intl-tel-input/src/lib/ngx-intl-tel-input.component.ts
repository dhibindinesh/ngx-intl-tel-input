...
export class NgxIntlTelInputComponent implements OnInit, OnChanges {
	...
	@Input() searchMode: "scroll" | "filter" = "filter";
  ...
  */
	...
	public searchCountry() {
		...
		if (this.searchMode === "filter") {
			// Filter by name only in filter mode
			this.filteredCountries = this.allCountries.filter((c) => c.name.toLowerCase().startsWith(countrySearchTextLower));
		}
		// @ts-ignore
		...
		if (this.filteredCountries.length > 0) {
			const el = this.countryList.nativeElement.querySelector("#" + this.filteredCountries[0].htmlId);
			if (el) {
				el.scrollIntoView({
					behavior: "smooth",
					block: "nearest",
